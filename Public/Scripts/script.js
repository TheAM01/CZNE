...
function productPage(socket) {
    ...
    socket.on("product", (response) => {
        ...
        const content = element("div", "prdt_content prdt_field", null, `Content:\n${data.content.join("\n")}`);
        const reviewsParent = element("div", "prdt_reviews_parent");
        const reviewHead = element("span", "heading", {style: "margin: 10px 0"}, "Reviews: ");
